...
Circular statistics are applied to a subset of `r length(X.circ$articulation)` isolated specimens (not articulated) from Geo 4, for which plunge,bearing and length were recorded and length >= 2cm. The subset of material was collected from 2015 to 2017.

The material includes 24 complete bones (1 femur, 12 metapode, 2 phalanx, 1 radius/fibula, 1 rib, 4 scapula, 2 tarsal, 1 ulna) and 239 fragmented bones (64 ?, 1 ?femur, 1 antler, 1 cranium, 1 femur, 4 humerus, 132 limb bone, 1 mandible, 7 metapode, 3 pelvis, 5 radius, 7 rib, 5 scapula, 3 tarsal, 3 tibia, 1 vertebra).

Articulated bones are kept apart from the fabric analysis, because they require specific experimental tests and analysis...

```{r}
# subset long bones () and classify by size
...
The rose diagrams of the bearing show a bimodal distribution, with prevalent SE direction and another peak at SW. NE and SW orientations could be consistent with a prevalent SE direction of the flow, since long bones could roll orthogonally to the flow direction. Interesting, the W 4° deeping of the geological horizon is not reflected in a W orientation of the bones.
...
Bearing - Sample mean direction:`r mean(bearing.circ)`; Sample mean resultant length:`r rho.circular(bearing.circ)`; Angular variance:`r angular.variance(bearing.circ)`; Standard deviation: `r sd.circular(bearing.circ)`

Plunge - mean plunge: `r deg(circ.mean(plunge))`; dispersion: `r deg(circ.disp(plunge))`
...
Rayleigh test (p-value = `r r.test(bearing)$p.value`) reject the null hypothesis of uniformity.

Kuiper's Test of Uniformity reject as well the null hypothesis. Watson's goodness of fit test also reject the null hypothesis. Rao spacing test reject as well the null hypothesis of uniformity. All the omnibus tests support an anisotropic / multimodal distribution of the fossil sample, at the 99% significance level.
...
The Schmidt diagrams show a planar fabric of the sample distribution. The points plot on the edge of the equal area hemisphere, thus indicating 0-to-low degree of plunge/plunge.
...
![](../src/Woodcock1983.png)

Fig.3 Stepwise procedure for the S1/S3 test. Modified from [@Woodcock1983]
...
The Benn diagram plots the distribution in the planar corner, with a very low isotropic index (`r is`) and elongation index (`r el`).
...
## Local Circular statistics

Fabric is an inherently spatially and spatiotemporally occurring phenomenon and eventally do not spread evenly across space.
...
X.circ.sp <- X.circ
X.circ.sp$y <- -X.circ.sp$y
coordinates(X.circ.sp) <- c("y", "x")

## distribution map
spplot(X.circ.sp, "bearing")

## spatial correlation of circular data?
plot(variogram(object=bearing~1, data=X.circ.sp, cloud=TRUE))
plot(variogram(object=bearing~1, data=X.circ.sp))

plot(variogram(bearing~1, X.circ.sp, alpha=c(0,45,90,135)))
```
